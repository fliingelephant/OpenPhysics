...
<problem id="QI005">
  ...
  <claim>
    ...
    \]
    Decide whether \(\rho\) is \(2\)-copy distillable. Equivalently, decide
    whether there exist rank-two projections \(P=P^\dagger=P^2\) and
  ...
  <known>
    The published PRX Quantum paper states Problem 5 as deciding whether the
    two-ququart Werner state \(\rho(4,-1/2)\) is \(2\)-copy distillable. In the
    Werner family, \(\rho(d,\alpha)\) is NPT for
    \(\alpha \in [-1,-1/d)\) and is \(1\)-copy nondistillable for
    \(\alpha \in [-2/d,1]\). Thus \(d=4,\alpha=-1/2\) is both NPT and at the
    boundary of \(1\)-copy nondistillability. Werner states are also called
    \(U \otimes U\)-invariant states because they are invariant under twirling
    by local unitaries.

    The source records the conjecture that Werner states which are not
    \(1\)-copy distillable are completely nondistillable; the claim here is
    only the special \(d=4,\alpha=-1/2\), two-copy case. The same source also
    records an equivalent algebraic formulation: prove that the sum of squares
    of the two largest singular values of
    \[
    ...
    additional assumption that both \(A\) and \(B\) are normal; the PRX Quantum
    source reports further progress for the case where one of the two matrices
    is arbitrary.
  </known>
  ...
  <ask>
    Decide the claim by either proving positivity of the compressed operator
    for all rank-two projections \(P,Q\), or by giving a specific pair \(P,Q\)
    for which it has a strictly negative eigenvalue. Equivalently, using the
    cited algebraic reduction, decide the stated singular-value bound for all
    admissible traceless matrices \(A,B\): proving the bound proves
    non-\(2\)-copy distillability, while an admissible pair \(A,B\) for which
    the sum of squares of the two largest singular values is greater than
    \(1/2\) proves \(2\)-copy distillability.
  </ask>
